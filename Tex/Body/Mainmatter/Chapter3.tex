...
\begin{itemize}
    \item The modulus $q = 3329$ is prime and satisfies $q \equiv 1 \pmod{256}$. This value is calculated as $q = 13 \times 256 + 1 = 3329$, chosen to be the smallest prime of this form that provides sufficient security. The condition $q \equiv 1 \pmod{256}$ ensures the existence of a primitive 256-th root of unity in $\mathbb{Z}_q$, which is required for the NTT to work correctly with 256-coefficient polynomials \cite{Kyber-Spec}.
    \item Kyber uses an \textit{incomplete NTT}. Instead of splitting into linear factors, the polynomial $X^{256} + 1$ decomposes into 128 polynomials of degree 2. Pointwise multiplication in the NTT domain is not performed coefficient-wise, but rather involves multiplying 128 pairs of degree-1 polynomials \cite{Kyber-Spec}.
...
Our implementation uses pure Python with NumPy for numerical operations. While Python is not traditionally known for high-performance cryptographic implementations, it provides excellent readability and maintainability for educational purposes. The implementation focuses on correctness and clarity of the mathematical operations rather than raw performance.

All polynomial arithmetic, NTT transformations, and modular operations are implemented using NumPy arrays and standard Python operations, ensuring the code is accessible for learning and verification purposes. This approach allows for straightforward debugging and modification while maintaining reasonable performance for demonstration and testing scenarios.
...
\begin{enumerate}
\item Client 1 (receiver) generates public-private key pair and sends public key to server
\item Server caches and forwards Client 1's public key to all connected clients
\item Client 2 (sender) receives Client 1's public key from server
\item Client 2 performs encapsulation using Client 1's public key, producing ciphertext and secret
...
\item Client 1 decapsulates ciphertext using their private key to recover the secret
\item Both clients independently derive the same AES encryption key one from generation, one from decapsulation.
\end{enumerate}
