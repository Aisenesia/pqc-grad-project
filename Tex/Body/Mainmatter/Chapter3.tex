...
Kyber uses SHAKE-128, a member of the SHA-3 family, as an extendable output function (XOF) for generating pseudorandom data. Unlike traditional hash functions with fixed output length, XOFs can produce arbitrary-length outputs from a short seed.

The primary application is generating the public matrix $\mathbf{A}$:
\begin{itemize}
    \item Instead of transmitting the entire matrix (requiring $k \times k \times 256$ coefficients, approximately 1.5 KB for Kyber-512), only a 32-byte seed is exchanged.
    \item Both parties deterministically expand this seed using SHAKE-128 to reconstruct the identical matrix $\mathbf{A}$.
    \item This reduces the matrix representation from $\approx$1536 bytes to 32 bytes, keeping the total public key size at 800 bytes \cite{Kyber-Spec}.
\end{itemize}
